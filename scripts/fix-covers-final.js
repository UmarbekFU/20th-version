...
// Only use URLs that we have verified work
const verifiedWorkingCovers = {
...
// Use a different working fallback URL
...
Object.entries(verifiedWorkingCovers).forEach(([title, coverUrl]) => {
...
// Replace any broken Amazon URLs with the fallback
content = content.replace(
  /coverImage: "https:\/\/m\.media-amazon\.com\/images\/I\/[^"]*"/g,
  (match) => {
    // Check if this is one of our known working URLs
    const isWorking = Object.values(verifiedWorkingCovers).some(url => match.includes(url));
    if (!isWorking) {
...
console.log('✅ Updated book covers with verified working URLs!');
console.log(`📚 Used specific URLs for ${Object.keys(verifiedWorkingCovers).length} books`);
