...
export default function SameasEverPage() {
  // All data is self-contained - no external dependencies
  const title = "Same as Ever"
  const author = "Morgan Housel"
  const date = "2024"
  const rating = 8
  const coverImage = "https://via.placeholder.com/300x400/%2310b981/ffffff?text=Same%20as%20Ever+by+Morgan%20Housel"
  const spineColor = "#ea580c"
  const textColor = "#ffffff"
  const summary = "A guide to what never changes. Housel's exploration of timeless principles in a changing world."
  const category = "book"
